...
# --- TARNKAPPEN SCRAPER (OPTIMIERT FÜR GEIZHALS) ---
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .topbar p {{ color: var(--text-muted); font-size: 15px; margin-top: 4px; }}
        ...
        .stat-label {{ font-size: 14px; color: var(--text-muted); font-weight: 500; text-transform: uppercase; }}
        .stat-value {{ font-size: 30px; font-weight: 700; margin: 12px 0 8px; letter-spacing: -0.02em; }}
        .stat-change {{ font-size: 13px; font-weight: 600; display: inline-flex; align-items: center; padding: 4px 10px; border-radius: 8px; background: rgba(48, 209, 88, 0.15); color: var(--green); }}
        .stat-change.blue {{ background: rgba(10, 132, 255, 0.15); color: var(--accent); }}

        /* Info Boxes */
        .ai-box {{ background: var(--surface-2); padding: 18px; border-radius: 12px; border-left: 4px solid var(--accent); line-height: 1.5; font-size: 14px; margin-bottom: 24px; }}
...
<body>
        ...
        <main class="main">
            <div class="topbar">
                <div>
                    ...
                    <p>Zuletzt synchronisiert: {now}</p>
                </div>
            </div>
            ...
            <div class="ai-box">
                <strong style="color: var(--accent);">🧠 Claude 3.5 Sonnet Kaufberatung:</strong><br><br>
                {decision}
            </div>

            <div class="card">
                <div class="card-header">Marktentwicklung (30 Tage)</div>
        ...
        for b in main_data['bundles']:
            html_content += f"""
                        <tr class="bundle-row">
                            ...
                            <td class="comp-price">{b['bundle_price']:.2f} €</td>
        ...
        let priceChart = new Chart(ctx, {{
            ...
            data: {{
                ...
                datasets: [{{
                    ...
                    data: rawHistory.map(i => i.main_total).slice(-30),
                    borderColor: '#0a84ff',
                    backgroundColor: 'rgba(10, 132, 255, 0.1)',
                    borderWidth: 2, tension: 0.4, fill: true, pointRadius: 0, pointHoverRadius: 6
                }}, {{
                    label: 'P/L-Sieger',
                    data: rawHistory.map(i => i.alt_total).slice(-30),
                    borderColor: '#30d158',
    ...
    print("HTML Dashboard im neuen v0-Design erfolgreich generiert.")
